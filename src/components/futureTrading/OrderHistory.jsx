...
const OrderHistory = ({ refreshTrigger = 0 }) => {
  ...
  const fetchOrderHistory = async () => {
    try {
      ...
      const apiKey =
        localStorage.getItem("apiKey") || "A20RqFwVktRxxRqrKBtmi6ud";
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        setOrderHistoryData(
          response.data.map((order) => ({ ...order, imgError: false }))
        );
      }
    } catch (err) {
  ...
  const confirmClosePosition = async () => {
    ...
    try {
      ...
      setError(
        "You cannot close position currently. Please contact customer service."
      );
  ...
  return (
    <div className="order-history-container dark-mode">
              ...
              {currentItems.length > 0
                ? currentItems.map((order) => (
                    <tr key={order._id} className="order-row-fadein">
                      ...
                      <td>
                        {Number(order.entry_price).toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </td>
                      <td>
                        {Number(order.margin).toLocaleString(undefined, {
                          minimumFractionDigits: 5,
                          maximumFractionDigits: 5,
                        })}
                      </td>
                      <td>
                        {Number(order.liquidation_price).toLocaleString(
                          undefined,
                      ...
                      <td>{order.cycle}d</td>
                      <td>
                        {Number(order.asset).toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </td>
                      <td>
                        {Number(order.return_percentage).toLocaleString(
                          undefined,
                          { minimumFractionDigits: 2, maximumFractionDigits: 2 }
                        )}
                        %
              ...
              {closeSuccess ? (
                <div className="text-center py-4">
                  ...
                  <p className="text-gray-400 text-sm">
                    Your position has been closed and funds have been returned
                    to your wallet.
                  </p>
                </div>
              ) : (
                <>
                  <div className="mb-5 text-center">
                    ...
                    <p className="text-gray-400 text-sm">
                      Are you sure you want to close this position? This action
                      cannot be undone.
                    </p>
                  </div>
                  ...
                  <div className="bg-gray-800 bg-opacity-50 rounded-lg p-4 mb-5">
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      ...
                      <div className="text-white font-medium">
                        $
                        {Number(selectedPosition.entry_price).toLocaleString(
                          undefined,
                          { minimumFractionDigits: 2, maximumFractionDigits: 2 }
                        )}
                      </div>
                      ...
                      <div className="text-white font-medium">
                        $
                        {Number(selectedPosition.asset).toLocaleString(
                          undefined,
                          { minimumFractionDigits: 2, maximumFractionDigits: 2 }
                        )}
                      </div>
                      ...
                      <div className="text-white font-medium">
                        {Number(
                          selectedPosition.return_percentage
      ...
      {showNotification && apiResponse && (
        <div
          className={`fixed bottom-6 right-6 max-w-md p-4 rounded-lg shadow-lg border-l-4 ${
            apiResponse.success
              ? "bg-[#181A20] border-green-500"
              : "bg-[#181A20] border-blue-500"
          ...
          <div className="flex items-start">
            ...
            >
              <FontAwesomeIcon
                ...
                className={`${
                  apiResponse.success ? "text-green-500" : "text-blue-500"
                } text-lg`}
              />
            </div>
