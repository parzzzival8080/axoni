...
const OrderBook = ({ cryptoData, forceRefresh = 0 }) => {
  ...
  const processOrderBookData = useCallback((asks, bids) => {
    if (!asks || !bids || !Array.isArray(asks) || !Array.isArray(bids)) {
      console.error("[OrderBook] Invalid order book data structure for FLUX", {
        asks,
        bids,
      });
      return null;
    ...
    const processedAsks = asks
      .map((item) => ({
        price: cryptoData.price, // FLUX: [price, size, liquidations, orders]
  ...
  const fetchOrderBookREST = useCallback(
    async (retry = 0) => {
      ...
      const fetchData = async () => {
        try {
            ...
            if (bookData.asks && bookData.bids) {
              ...
              if (processedData) {
                ...
                const hasSignificantChanges =
                  ...
                  JSON.stringify(processedData.asks.slice(0, 3)) !==
                    JSON.stringify(orderBook.asks.slice(0, 3)) ||
                  JSON.stringify(processedData.bids.slice(0, 3)) !==
                    JSON.stringify(orderBook.bids.slice(0, 3));
              ...
              throw new Error(
                "Malformed FLUX REST API response data structure"
              );
            }
          } else {
            throw new Error(
          ...
          console.error(
            `[OrderBook] FLUX REST API fetch error (attempt ${retry + 1}):`,
            error
          );
          if (retry < MAX_REST_RETRIES) {
            setTimeout(
              () => fetchOrderBookREST(retry + 1),
              REST_BACKOFF_BASE * Math.pow(2, retry)
            );
          } else {
            ...
            setOrderBook(
              generateMockOrderBook(parseFloat(cryptoData?.cryptoPrice))
            ); // Fallback to mock
          }
        }
      };
  ...
  const connectWebSocket = useCallback(() => {
    ...
    try {
      ...
      socket.onopen = () => {
        ...
        console.log(
          "[OrderBook] Sent FLUX subscription message:",
          subscriptionMessage
        );
      ...
      socket.onclose = (event) => {
        console.log(
          "[OrderBook] FLUX WebSocket closed:",
          event.code,
          event.reason
        );
        ...
        if (
          ...
          // Avoid if manually closed or already failed
          if (reconnectAttempts < maxReconnectAttempts) {
            ...
            console.log(
              `[OrderBook] Attempting to reconnect to FLUX in ${
                backoffTime / 1000
              }s (attempt ${reconnectAttempts + 1})...`
            );
            ...
            console.error(
              "[OrderBook] Max reconnect attempts to FLUX reached. Falling back to REST."
            );
      ...
      socket.onmessage = (event) => {
        try {
          ...
          if (message.event === "subscribe") {
            console.log(
              "[OrderBook] FLUX Subscription confirmed:",
              message.arg
            );
            return;
          }
          if (message.event === "error") {
            console.error(
              "[OrderBook] FLUX API Error Message:",
              message.msg,
              "Code:",
              message.code
            );
          ...
          console.error(
            "[OrderBook] Error processing FLUX WebSocket message:",
            error,
            event.data
          );
        }
  ...
  useEffect(() => {
    ...
    if (canUseWebSocket) {
      ...
      console.log(
        "[OrderBook] WebSocket not supported, falling back to REST for FLUX."
      );
      setConnectionStatus("fallback");
      fetchOrderBookREST();
    }
    ...
    staleConnectionCheckRef.current = setInterval(() => {
      if (
        ...
        Date.now() - lastUpdateTimeRef.current > 20000
      ) {
        // 20s no data
        console.warn(
          "[OrderBook] FLUX WebSocket connection appears stale (no data for >20s). Closing to trigger reconnect."
        );
        wsRef.current.close(); // This will trigger the onclose logic for reconnection
      } else if (
        connectionStatus === "fallback" &&
        Date.now() - lastUpdateTimeRef.current > 30000
      ) {
        // 30s for REST
        console.warn(
          "[OrderBook] FLUX REST connection appears stale. Re-fetching."
        );
    ...
    (basePrice = 60000) => {
      ...
      return {
        asks: Array(8)
          .fill()
          .map((_, i) => ({
            price: askBasePrice + i * (basePrice * 0.0001),
        ...
        bids: Array(8)
          .fill()
          .map((_, i) => ({
            price: bidBasePrice - i * (basePrice * 0.0001),
            amount: 0.1 + Math.random() * 0.3,
            total: 0,
            percentage: 0,
          }))
  ...
  const handlePrecisionChange = useCallback(
    (precision) => setDecimalPrecision(precision),
    []
  );
  ...
  const ensureExactRows = useCallback((data, count, isAsk) => {
    ...
    if (data.length < count) {
      ...
      const padding = Array(count - data.length)
        .fill(null)
        .map((_, i) => {
  ...
  const displayAsks = useMemo(
    () => ensureExactRows(orderBook.asks, 8, true),
    [orderBook.asks, ensureExactRows]
  );
  const displayBids = useMemo(
    () => ensureExactRows(orderBook.bids, 8, false),
    [orderBook.bids, ensureExactRows]
  );
      ...
      <div className="order-book-header">
        ...
        <div className="decimals">
          {[0, 1, 2, 3, 4].map((p) => (
            <div
              key={p}
              className={`decimal ${decimalPrecision === p ? "active" : ""}`}
          ...
          orderBook.bids.length === 0 ? (
            <>
              ...
              <div className="current-price">
                <div
                  className="price-value skeleton-cell skeleton-rect"
                  style={{ width: 80, height: 18, margin: "0 auto" }}
                />
                <div
                  className="price-usd skeleton-cell skeleton-rect"
                  style={{ width: 60, height: 11, margin: "6px auto 0" }}
                />
              </div>
            ...
            <>
              <div className="sell-orders">
                {displayAsks.map((ask, index) => (
                  <div
                    className="order-row"
                    key={`ask-${ask?.price || index}-${index}`}
                  >
                    <div
                      className="order-bar sell"
                      style={{ width: `${ask?.percentage || 0}%` }}
                    ></div>
                    <div
                      className="order-price sell"
                      style={{ opacity: ask?.isEmpty ? 0.5 : 1 }}
                    >
              ...
              <div className="current-price">
                <div className="price-value">
                  {currentPrice
                    ? formatNumber(currentPrice, decimalPrecision)
                    : "-.--"}{" "}
                  {usdtSymbol}
                </div>
                <div className="price-usd">
                  {/* Assuming 1 USDT = 1 USD for simplicity */}$
                  {currentPrice ? formatNumber(currentPrice * 1, 2) : "-.--"}{" "}
                  USD
                </div>
              </div>
              ...
              <div className="buy-orders">
                {displayBids.map((bid, index) => (
                  <div
                    className="order-row"
                    key={`bid-${bid?.price || index}-${index}`}
                  >
                    <div
                      className="order-bar buy"
                      style={{ width: `${bid?.percentage || 0}%` }}
                    ></div>
                    <div
                      className="order-price buy"
                      style={{ opacity: bid?.isEmpty ? 0.5 : 1 }}
                    >
