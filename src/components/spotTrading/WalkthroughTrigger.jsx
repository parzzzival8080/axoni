  ...
  return (
    <>
      {!showFloatingCard ? (
        ...
        <div className="fixed bottom-6 right-6 w-80 bg-gradient-to-br from-zinc-900 to-zinc-800 border border-yellow-500/30 rounded-2xl shadow-2xl backdrop-blur-xl text-white z-[1000] animate-in slide-in-from-bottom-4 duration-400 md:w-72 sm:bottom-5 sm:right-5 sm:left-5 sm:w-auto">
          ...
          <div className="p-5">
            <p className="mb-5 text-sm leading-relaxed text-white/80">
              New to AXONI? Take our guided tour to learn all the features!
            </p>
            
            <div className="space-y-4">
              <button 
                onClick={handleOpenWalkthrough}
                className="w-full bg-gradient-to-r from-yellow-500 to-yellow-400 text-white font-semibold px-5 py-3 rounded-lg hover:shadow-lg hover:shadow-yellow-500/25 hover:-translate-y-0.5 transition-all duration-200"
              >
                Start Tour
              </button>
              
              <div className="space-y-2">
                <div className="text-xs leading-tight text-white/70 px-3 py-2 bg-white/5 rounded border-l-3 border-yellow-500/50">
                  <strong className="text-yellow-500">💡 Quick Tip:</strong> Use the percentage slider for easy amount selection
                </div>
                {!hasSeenWalkthrough && (
                  <div className="text-xs leading-tight text-white/90 px-3 py-2 bg-yellow-500/10 border border-yellow-500/20 rounded border-l-3 border-yellow-500">
                    <strong className="text-yellow-500">👋 Welcome!</strong> We recommend taking the tour first
                  </div>
                )}
              </div>
            </div>
