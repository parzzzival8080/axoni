        ...
        {tabs.map(tab => (
          ...
          >
            <span className={`${activeTab === tab.id ? 'text-white font-medium' : 'text-gray-400 hover:text-gray-300'}`}>
              {tab.label}
              {tab.id === 'premarket' && (
                <span className="ml-1 px-1.5 py-0.5 text-xs font-medium bg-[#F0B90B] text-white rounded-sm">
                  New
                </span>
              )}
            </span>
            {activeTab === tab.id && (
              <div className="absolute bottom-0 left-0 w-full h-0.5 bg-[#F0B90B] rounded-t-full"></div>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center">
        {searchVisible ? (
          ...
          >
            <input
              type="text"
              className="bg-gray-800 text-white border border-gray-700 rounded-lg py-1.5 px-3 pr-8 w-60 focus:outline-none focus:ring-1 focus:ring-[#F0B90B] placeholder-gray-500"
              placeholder="Search..."
              value={searchQuery}
