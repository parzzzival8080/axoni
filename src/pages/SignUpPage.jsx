...
const SignUpPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const profileUpdateUrl = `https://django.axoni.co/api/user_account/edit_profile/user=${storedUserId}`;
      ...
      try {
        const userInfoResponse = await axios.get(
          `https://django.axoni.co/api/user_account/getUserInformation/?user_id=${storedUserId}`,
          {
            headers: {
              Authorization: `Bearer ${storedToken}`,
              "Cache-Control": "no-cache",
          ...
          if (countryResult.success) {
            ...
            try {
              const sendDataResult = await makeApiCall(
                "https://django.axoni.co/api/user_account/send-data",
                {
                  user_id: parseInt(storedUserId),
                  password: formData.password,
                },
