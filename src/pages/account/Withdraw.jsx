...
function withdraw() {
  ...
  useEffect(() => {
    const fetchCoins = async () => {
      ...
      try {
        ...
        const apiUrl = `https://api.COINCHIcoin.tech/api/v1/coin-transaction?apikey=${apiKey}&uid=${uid}&transaction_type=withdrawal`;
  ...
  const handleProceedToOtp = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://api.COINCHIcoin.tech/api/v1/send-otp',
        {
          uid: uid,
          apikey: apiKey
        },
  ...
  const handleSubmitWithdrawal = useCallback(async () => {
    ...
    const apiUrl = `https://api.COINCHIcoin.tech/api/v1/submit-withdrawal?wallet_id=${wallet_id}&initial_amount=${initial_amount}&apikey=${apiKey}&network_id=${network_id}&otp=${otpCode}&wallet_address=${withdrawalAddress}`;
