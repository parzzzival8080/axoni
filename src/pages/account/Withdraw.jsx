...
function withdraw() {
  ...
  const [withdrawalDestination, setWithdrawalDestination] =
    useState("on-chain"); // 'on-chain' or 'internal'
  ...
  const faqQuestions = [
    {
      ...
      slug: "how-to-make-withdrawal",
    },
    {
      id: 2,
      question: "Why have I still not received my withdrawal?",
      slug: "withdrawal-not-received",
    },
    {
      id: 3,
      question:
        "How do I select the correct network for my crypto withdrawals and deposits?",
      slug: "select-correct-network",
    },
    {
      id: 4,
      question: "Do I need to pay fees for deposit and withdrawal?",
      slug: "withdrawal-deposit-fees",
    },
  ];
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        addressBookDropdownRef.current &&
        !addressBookDropdownRef.current.contains(event.target)
      )
  ...
  const networkOptions = useMemo(() => {
    if (!selectedCoinDetails || !Array.isArray(selectedCoinDetails.network))
      return [];
  ...
  useEffect(() => {
    ...
    setAvailableBalance(
      coin && coin.balance && coin.balance.spot_wallet
        ? Number(coin.balance.spot_wallet)
        : 0,
    );
  }, [selectedCryptoSymbol, coins]);
  ...
  const handleNetworkSelect = useCallback((network) => {
    // network is now the full network object
  ...
  const handleAddressBookSelect = useCallback(
    (addressEntry) => {
      ...
      const matchingNetwork = networkOptions.find(
        (n) => n.value === addressEntry.network,
      );
      if (matchingNetwork) {
  ...
  const handleProceedToOtp = useCallback(async () => {
    ...
    if (!addressValidation.isValid || addressValidation.error) {
      ...
      setSubmitError(
        addressValidation.error ||
          "Please enter a valid address for the selected network.",
      );
      return;
    }
    ...
    if (amountNum < networkFee) {
      ...
      setSubmitError(
        `Withdrawal amount must be greater than the network fee (${networkFee} ${selectedCryptoSymbol}).`,
      );
      return;
    }
  ...
  // Handle OTP verification and withdrawal submission
  const handleVerifyOtp = useCallback(async () => {
    ...
    try {
      ...
      if (!wallet_id || !initial_amount || !network_id) {
        throw new Error('Missing withdrawal parameters. Please try again.');
      }
      ...
      // Check if withdrawal failed due to invalid OTP
      ...
      // Check for successful withdrawal response
      if (
        response.data && (
      ...
      console.error('Withdrawal OTP verification error:', error);
      ...
      if (error.response && error.response.data) {
        if (error.response.data.message) {
          if (error.response.data.message.toLowerCase().includes('invalid otp')) {
  ...
  }, [otpCode, selectedCoinDetails, withdrawalAmount, selectedNetwork, withdrawalAddress]);
  ...
  const handleResendOtp = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.error && response.data.status === 400) {
        ...
        if (response.data.error === "Failed to send email.") {
          errorMessage =
            "Failed to resend verification code to your email. Please try again later.";
        } else {
          errorMessage = response.data.error;
        }
  ...
  // This function is no longer needed as OTP verification and withdrawal submission are combined
  // Keeping it for backward compatibility, but it's not used
  const handleSubmitWithdrawal = useCallback(async () => {
    // This function is now integrated into handleVerifyOtp
    console.log('handleSubmitWithdrawal is deprecated - use handleVerifyOtp instead');
  }, []);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <section>
                ...
                <div
                  className={`${showOtpStep ? "opacity-50 pointer-events-none" : ""}`}
                >
                  ...
                  <div className="mb-4">
                    ...
                    {withdrawalAddress && addressValidation.error && (
                      <div className="mt-2 flex items-start space-x-2">
                        <FontAwesomeIcon
                          icon={faTimes}
                          className="text-red-400 mt-0.5 text-xs flex-shrink-0"
                        />
              ...
              <section>
                ...
                <div
                  className={`${activeStep >= 3 && !showOtpStep ? "opacity-100" : "opacity-50 pointer-events-none"}`}
                >
                  <div className="mb-4">
                      ...
                      <div className="flex items-center">
                        ...
                        <div className="ml-3">
                          ...
                          <p className="mt-1 text-sm text-green-700">
                            Your withdrawal has been submitted and is being
                            processed. You can track its progress in the
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex space-x-3">
                            ...
                            <button
                              ...
                              className="flex-1 h-11 flex items-center justify-center px-4 py-2 bg-[#F88726] text-white text-sm font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#F88726] hover:bg-[#ff9c44] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                            >
                              {isVerifyingOtp ? (
                                <Spinner />
                              ) : (
