...
import FileUploadButton from "../../components/account/FileUploadButton";
...
const InitialVerifyIllustration = () => (
 ...
 <circle cx="60" cy="48" r="20" fill="#2A2A2A" />
 <rect x="34" y="76" width="52" height="16" rx="8" fill="#2A2A2A" />
 <rect x="50" y="98" width="20" height="6" rx="3" fill="#2A2A2A" />
 </svg>
);
...
const VerifyPage = () => {
 ...
 isValid: false,
 error:
 "Unsupported file type. Please use PNG, JPEG, JPG, or WebP format.",
 };
 }
 ...
 async (file, maxWidth = 1200, quality = 0.8) => {
 ...
 idBack &&
 idBack.size > API_CONFIG.COMPRESSION_THRESHOLD
 ) {
 ...
 if (
 currentStep === 4 &&
 (!idFront || (selectedIdInfo?.needsBack && !idBack))
 )
 return true;
 ...
 <p className="text-[#848E9C] mb-6 text-center">
 Your information has been submitted and is currently under review.
 You will be notified by email once your account is verified. This
 may take up to 24 hours.
 </p>
 <Link
 to="/"
 className="px-6 py-2 rounded-full bg-[#2EBD85] text-white font-semibold text-sm hover:bg-[#259A6C] transition shadow-lg"
 ...
 <p className="text-[#848E9C] mb-6 text-center">
 Thank you for verifying your identity. Your account is fully
 verified and you have access to all features.
 </p>
 <Link
 to="/account/profile"
 className="px-6 py-2 rounded-lg bg-[#2EBD85] text-white font-semibold text-sm hover:bg-[#259A6C] transition"
 >
 Go to Profile
 </Link>
 </div>
 ...
 <p className="text-[#848E9C] mb-6 text-center">
 Your verification was not approved. Please ensure all documents are
 clear and information is accurate, then try again.
 </p>
 <button
 onClick={() => {
 setIsRetrying(true);
 ...
 const progress =
 currentStep > 1 && currentStep <= totalStepsForProgress + 1
 ? ((currentStep - 1) / totalStepsForProgress) * 100
 : 0;
 ...
 <p className="mt-2 text-sm text-[#848E9C] max-w-sm">
 To comply with regulations and ensure the security of your
 account, please complete the identity verification process.
 </p>
 </div>
 <div className="w-full bg-[#1E1E1E] p-5 rounded-xl mb-6">
 ...
 <p className="text-xs text-[#5E6673] mt-0.5">
 Suitable for personal use. You'll need a valid ID and a
 selfie. The process typically takes a few minutes.
 </p>
 </div>
 </div>
 </div>
 ...
 <p className="text-sm text-[#5E6673] mb-5">
 ...
 className="w-full p-3.5 pr-10 border border-[#2A2A2A] rounded-lg bg-[#1E1E1E] text-white focus:ring-2 focus:ring-black focus:border-black outline-none appearance-none text-sm"
 disabled={isLoading}
 translate="no"
 >
 ...
 <ChevronRight
 size={18}
 className="absolute right-3 top-1/2 -translate-y-1/2 text-[#5E6673] pointer-events-none"
 />
 </div>
 <div className="mt-4 text-xs text-[#5E6673]">
 <p className="font-medium text-[#848E9C]">
 This helps us provide you with the right services. You'll need
 to provide:
 </p>
 ...
 <ChevronRight size={18} className="text-[#5E6673]" />
 </div>
 </button>
 ))}
 </div>
 ...
 icon={<ImagePlus size={32} className="mb-2 text-[#5E6673]" />}
 ...
 className="max-w-full h-auto max-h-48 inline-block rounded border border-[#2A2A2A]"
 onError={(e) => {
 ...
 icon={
 <ImagePlus size={32} className="mb-2 text-[#5E6673]" />
 }
 ...
 <p className="text-sm text-[#5E6673] mb-5">
 Position your face in the frame. Ensure good lighting and no
 accessories like glasses or hats.
 </p>

 {/* Camera/Image Preview Area */}
 <div className="w-full aspect-square max-w-[280px] mx-auto bg-[#2A2A2A] rounded-full flex items-center justify-center mb-5 relative overflow-hidden">
 {isCameraMode && !selfie ? (
 <video
 ref={videoRef}
 ...
 <Camera size={56} className="text-[#5E6673]" />
 )}
 ...
 disabled={isLoading}
 className="absolute bottom-4 left-1/2 transform -translate-x-1/2 w-16 h-16 bg-[#1E1E1E] rounded-full border-4 border-[#2A2A2A] flex items-center justify-center shadow-lg hover:bg-[#1E1E1E] transition-colors disabled:opacity-50"
 >
 ...
 .<br />
 You will receive an email once your verification has been
 processed. This may take up to 24 hours.
 </p>
 <button
 onClick={() => {
 ...
 {/* Main content area */}
 <div
 className={`w-full bg-[#1E1E1E] rounded-xl
 ${
 currentStep === 1 || currentStep === 6
 ...
 {currentStep < 6 && (
 <div className="text-xs text-[#5E6673] mt-6 text-center max-w-xs mx-auto">
 <Info size={14} className="inline mr-1 mb-0.5 text-[#5E6673]" />
 Your information is encrypted and stored securely. It will only be
