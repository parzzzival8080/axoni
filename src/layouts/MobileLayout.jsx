...
const MobileLayout = ({ children, title, actions, darkBg = false, hideTabBar = false, hideHeader = false }) => {
  return (
    <div className={`min-h-screen ${darkBg ? "bg-black" : "bg-white dark:bg-gray-900"}`}>
      {!hideHeader && <MobileHeader title={title} actions={actions} darkBg={darkBg} />}
      <ToastNotification />
      <main
        className="overflow-y-auto"
        style={{
          paddingTop: hideHeader ? "0px" : "calc(48px + env(safe-area-inset-top, 0px))",
          paddingBottom: hideTabBar ? "0px" : "calc(56px + env(safe-area-inset-bottom, 0px))",
          minHeight: "100vh",
        }}
      >
        {children}
