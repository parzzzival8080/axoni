...
export class HistoryProvider {
    ...
    getBars(symbolInfo, resolution, periodParams) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            // if (historicalData) {
            //     // If historical data exists, resolve with the parsed data
            //     const parsedData = JSON.parse(historicalData);
            //     resolve(parsedData);
            //     const initialResponse = await this._requester.sendRequest(this._datafeedUrl, 'history/ticker', requestParams);
            //     // console.log(initialResponse.data.time)
            //     // latest = historicalData.bars[historicalData.bars.length - 1];
            //     const latest = parsedData.bars[parsedData.bars.length - 1]
            //     // console.log(latest.time)
            //     if (latest != initialResponse.data.time) {
            //         parsedData.bars.push(initialResponse.data)
            //         console.log(parsedData.bars[parsedData.bars.length - 1])
            //         localStorage.setItem('historicalData', JSON.stringify(parsedData));
            //         resolve(parsedData);
            //         await this._processTruncatedResponse(parsedData, requestParams);
            //     }
            ...
            try {
                ...
                else if (sessionStorage.getItem('slTimeFrame') == '1hr') {
                    const link = 'history/one-hour'
                ...
                else if (sessionStorage.getItem('slTimeFrame') == '4hr') {
                    const link = 'history/four-hour'
                ...
                else if (sessionStorage.getItem('slTimeFrame') == '1D') {
                    const link = 'history/one-day'
    ...
    async _processTruncatedResponse(result, requestParams) {
        ...
        try {
            while (this._limitedServerResponse &&
                ...
                const followupResponse = await this._requester.sendRequest(this._datafeedUrl, 'history/ticker', requestParams);
